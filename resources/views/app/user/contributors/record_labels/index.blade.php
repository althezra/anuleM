<x-app-layout>
    ...
    <x-slot name="header">
        <h1 class="text-4xl font-semibold">Record Labels</h1>
        ...
        {{-- CSS: force show view-more button on row hover (consistent with publishers) --}}
        <style>
            #recordLabelsTable tbody tr:hover .view-more-btn {
                opacity: 1 !important;
            }
        </style>
    </x-slot>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-1 gap-2">
                <div class="bg-[#F5F5F5] px-6 py-4 rounded-md flex flex-col gap-2">
                    ...
                    <h5 class="text-lg flex items-end gap-2">
                        <span class="text-3xl font-bold" id="labelCount">0</span>
                        Record Labels
                    </h5>
                </div>
            </div>
    ...
    <div x-data>
                    ...
                    <div x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0"
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                ...
                                <div class="w-full flex flex-col gap-2">
                                    <label class="text-xs xl:text-sm text-[#1F1F1F]">Instagram</label>
                                ...
                                <div class="w-full flex flex-col gap-2">
                                    <label class="text-xs xl:text-sm text-[#1F1F1F]">Facebook</label>
                                ...
                                <div class="w-full flex flex-col gap-2">
                                    <label class="text-xs xl:text-sm text-[#1F1F1F]">YouTube</label>
                ...
                <div class="relative bg-[#F5F5F5] px-5 py-5 rounded-md w-full max-w-md mx-auto transform transition-all"
                    ...
                    <div x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0"
                        ...
                        <div class="flex item-center justify-between mb-4">
                            <h2 class="text-xl font-semibold text-black">Delete Record Label
                            </h2>
                        ...
                        <div class="mb-4">
                            <p class="text-md font-semibold text-[#646464] ">Are you sure want to permanently delete
                                the Record Label <span class="font-bold">"<span
        ...
        <div x-show="$store.modals.createModalOpen" x-transition:enter="transition ease-out duration-150"
            ...
            <div class="flex items-center justify-center min-h-screen p-4">
                <div class="relative bg-[#F5F5F5] px-6 py-4 rounded-md flex flex-col gap-4 w-full max-w-xl mx-auto transform transition-all"
                    ...
                    <div x-show="$store.modals.loading"
        ...
        <div x-show="$store.modals.editModalOpen" x-transition:enter="transition ease-out duration-150"
            ...
            x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 transform"
            x-transition:leave-end="opacity-0 transform" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
            <div class="fixed inset-0 bg-black opacity-25 transition-opacity" @click="$store.modals.closeEditModal()">
            </div>
            <div class="flex items-center justify-center min-h-screen p-4">
                <div class="relative bg-[#F5F5F5] px-6 py-4 rounded-md flex flex-col gap-4 w-full max-w-xl mx-auto transform transition-all"
                    @click.outside="$store.modals.closeEditModal()">
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.store('modals', {
                    ...
                    recordLabels: [], // Array untuk data dari API
                    dataTable: null,
                    toast: {
                        show: false,
                        message: '',
                    ...
                    // ===== FETCH DATA =====
                    async fetchRecordLabels() {
                        try {
                            const res = await fetch('/api/record-labels', {
                                headers: {
                                    'Accept': 'application/json',
                                    'X-Requested-With': 'XMLHttpRequest'
                                }
                            });
                            if (!res.ok) throw new Error('Failed to fetch record labels');

                            this.recordLabels = await res.json();
                    ...
                    renderTable() {
                        const tbody = document.querySelector('#recordLabelsTable tbody');
                        if (!tbody) return;
                        tbody.innerHTML = '';

                        this.recordLabels.forEach(l => {
                            const tr = document.createElement('tr');
                            // Class styling disamakan
                            tr.className =
                                "border-b border-[#D6D6D6] group hover:bg-gray-50 transition duration-150";
                            const avatarUrl = "{{ route('avatar') }}?name=" + encodeURIComponent(l
                                .name);

                            // PERBAIKAN: SVG langsung dimasukkan ke dalam HTML string agar tidak hilang saat render
                            tr.innerHTML = `
                                <td>
                                    <div class="inline-flex justify-between items-center gap-4 w-full">
                                        ...
                                        <div class="flex h-15 items-center gap-2 transition duration-150 ease-in-out">
                                            <button type="button" class="view-more-btn opacity-0 group-hover:opacity-100 transition-all duration-200 rounded-full p-2 hover:bg-[#E0E0E0]/70 active:bg-[#E0E0E0] text-[#302995] cursor-pointer inline-flex items-center justify-center"
                                                data-id="${l.id}"
                                                data-name="${l.name}"
                                                data-website="${l.website || ''}"
                                                data-instagram="${l.instagram || ''}"
                                                data-facebook="${l.facebook || ''}"
                                                data-youtube="${l.youtube || ''}">
                                            </button>
                                        </div>
                                    </div>
                                </td>
                        ...
                        // NOTE: Logika binding click dipindahkan ke initializeDataTable (Re-bind logic)
                        // agar sesuai arsitektur Publisher dan tahan terhadap pagination/sorting
                    },

                    initializeDataTable() {
                        const tableEl = document.querySelector("#recordLabelsTable");
                        if (!tableEl) return;
                        ...
                        // Styling injection (sama seperti publisher/artist)
                        const applyCustomStyles = () => {
                            ...
                            // ===== RE-BIND LOGIC (Meniru Arsitektur Publisher) =====
                            // Ini dipanggil setiap kali tabel dirender ulang (page change, sort, search)
                            // untuk memastikan tombol titik tiga tetap berfungsi dan iconnya ada.
                            document.querySelectorAll('.view-more-btn').forEach(btn => {
                                // Pastikan icon SVG ada (Safety check)
                    ...
                    updateCount() {
                        const el = document.getElementById('labelCount');
                        if (el) el.textContent = this.recordLabels.length;
                    },
                    ...
                    async createRecordLabel(data) {
                        ...
                        try {
                            ...
                            if (!res.ok) {
                                ...
                                throw new Error(errorData.message || 'Failed to create record label');
                            }
                            const newLabel = await res.json();
                            this.recordLabels.push(newLabel);
                            this.sortRecordLabels();
                            this.updateDataTable();
                            this.closeCreateModal();
                            this.showToast('Record Label created successfully!', 'success');
                        } catch (err) {
                            console.error(err);
                    ...
                    async updateRecordLabel(id, data) {
                        this.loading = true;
                        try {
                            const res = await fetch(`/api/record-labels/${id}`, {
                                method: 'PUT',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                            ...
                            if (!res.ok) {
                                const errorData = await res.json();
                                throw new Error(errorData.message || 'Failed to update record label');
                            }
                            const updated = await res.json();
                            const i = this.recordLabels.findIndex(l => l.id == id);
                            if (i !== -1) this.recordLabels[i] = updated;

                            this.sortRecordLabels();
                            this.updateDataTable();
                            this.closeEditModal();
                            this.showToast('Record Label updated successfully!', 'success');
                        } catch (err) {
                            console.error(err);
                            this.showToast(err.message, 'error');
                        } finally {
                    ...
                    submitTakedown() {
                        ...
                        fetch('/api/record-labels/' + this.deleteId, {
                            method: 'DELETE',
                            headers: {
                                'X-Requested-With': 'XMLHttpRequest',
                                'X-CSRF-TOKEN': token,
                            ...
                            if (res.ok) {
                                ...
                                this.recordLabels = this.recordLabels.filter(p => p.id != this
                                    .deleteId);
                                this.updateDataTable();
                                this.closeViewModal();
                                ...
                                this.closeDeleteModal();
                                this.showToast('Failed to delete record label', 'error');
                            }
                        }).catch(err => {
                            console.error(err);
                            this.closeDeleteModal();
            ...
            document.addEventListener('DOMContentLoaded', () => {
                Alpine.store('modals').fetchRecordLabels();
                ...
                if (createForm) {
                    createForm.addEventListener('submit', async function(e) {
                        ...
                        await Alpine.store('modals').createRecordLabel(data);
                        this.reset();
                    });
                }
                ...
                if (editForm) {
                    editForm.addEventListener('submit', function(e) {
                        ...
                        Alpine.store('modals').updateRecordLabel(id, data);
                    });
                }
            });
        </script>
